    ...
    def _retry_and_parse(self, prompt, parser, topic, difficulty):
        for attempt in range(settings.MAX_RETRIES):
            try:
                ...
                self.logger.info("Sucessfully generated and parsed question.")
                return parsed
            except Exception as e:
                self.logger.error(f"Error during generation/parsing: {e}")
    ...
    def generate_fill_blank(self, topic: str, difficulty: str = 'medium') -> FillBlankQuestion:
        try:
            ...
            if '____' not in question.question:
                raise ValueError("Invalid Fill-in-the-Blank Structure.")
            
            self.logger.info(f"Generated a valid Fill in Blank question.")
